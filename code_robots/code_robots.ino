#define MOTOR_SPEED 24
#define MOTOR_SPEED_TURN 26
...
#define BATTERY_LOW_VOLTAGE_CUTOFF 6.9
...
void loop()
{
   // BT.print("\r\n");
   // BT.print(i);
    int ir_left_status = digitalRead(IR_LEFT_PIN_NO);
    int ir_right_status = digitalRead(IR_RIGHT_PIN_NO);
    if(batteryVoltageCheck()==true)
    {
      if(ir_left_status==BLACK &&  ir_right_status==BLACK)
      {
        motionStop();
        beep (3,500);
        //Serial.println("STOP\r\n");
      }
      else if(ir_left_status==WHITE&&  ir_right_status==WHITE)
      {
        motionForward(MOTOR_SPEED);
         //Serial.println("GO\r\n");
      }
      else if(ir_left_status==BLACK &&  ir_right_status==WHITE)
      {
        motionLeft(MOTOR_SPEED_TURN); 
        //Serial.println("LEFT\r\n");      
        
      }
      else if(ir_left_status==WHITE &&  ir_right_status==BLACK)
      {
         motionRight(MOTOR_SPEED_TURN);
